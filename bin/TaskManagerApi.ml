...
module TaskManagerData : TaskManagerData = struct 
  ...
  let add_task manager name task = 
    manager.existing_tasks <- (name, task) :: manager.existing_tasks;
    Lwt.return ()
  ...
  let tasks manager = manager.existing_tasks |> List.mapi (fun i (n, t) -> (i, n, t)) |> Lwt.return 
...
let list_index_name_to_json (item_jsonifier : int * string * 'a -> string ) (list : (int * string * 'a) list) : string = 
  "["
  ^ (list 
...
let api_calls (scheduler_mvar : Scheduler.message_sender) (task_manager : TaskManagerData.t) = [
  ...
  *)
  Dream.post "/api/new_task" (fun req -> 
    ...
    let* task = 
      TaskManager.instantiate 
        ...
        (snd task_type)
    in 
    let* () = TaskManagerData.add_task task_manager decoded_body.task_name task in
    Dream.json "{}");
  (*
    Accepts body
    {
  ...
  Dream.post "/api/modify_task" (fun req -> 
    ...
    let* () = TaskManagerData.add_task task_manager task_name new_task in
    Dream.json "{}");
  (*
    Accepts body
    {
  ...
  Dream.post "/api/delete_task" (fun req -> 
    ...
    in 
    let* (_, old_task) = TaskManagerData.task_by_id task_manager decoded_body.task_id in 
    (* delete old task from list*)
    let* () = TaskManagerData.remove_task task_manager decoded_body.task_id in 
